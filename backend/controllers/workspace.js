...
const createWorkspace = async (req, res) => {
    try {
        ...
        console.log(error)
        return res.status(500).json({ message: "Internal server mengalami kemarau pada backend controllers -> createWorkspace -> workspace.js " })
    }
}
const getWorkspaces = async (req, res) => {
    try {
        ...
        console.log(error)
        return res.status(500).json({ message: "Internal server mengalami kekacauan di backend controllers -> getWorkspaces -> workspace.js" })
    }
}
const getWorkspaceDetails = async (req, res) => {
    try {
        ...
        if (!workspace) {
            return res.status(404).json({ message: "Tidak ditemukan Id Workspace detail" })
        }
        res.status(200).json(workspace)
    } catch (error) {
        console.log(error)
        return res.status(500).json({ message: "Internal server mengalami kekacauan di backend controllers -> getWorkspaceDetails -> workspace.js" })
    }
}
const getWorkspaceProjects = async (req, res) => {
    try {
        ...
        if (!workspace) {
            return res.status(404).json({ message: "Tidak ditemukan Id Workspace projects" })
        }
        ...
        console.log(error)
        return res.status(500).json({ message: "Internal server mengalami kekacauan di backend controllers -> getWorkspaceProjects -> workspace.js" })
    }
}
...
<div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px;">
    ...
    <a href="https://yourwebsite.com" target="_blank">
        <img src="https://yourwebsite.com/logo.png" alt="KPMIBM-R" style="height: 50px;" />
    </a>
    </div>

    <!-- Heading -->
    <h2 style="text-align: center; color: #2c3e50;">You're Invited!</h2>
    ...
    <p><a href="${invitationLink}" style="color: #4f46e5;">${invitationLink}</a></p>

    <!-- Footer -->
    <hr style="margin: 30px 0; border: none; border-top: 1px solid #ddd;" />
    <p style="font-size: 12px; color: #777; text-align: center;">
    You received this email because someone invited you to join a workspace on <a href="https://kpmibm-r.com" style="color: #4f46e5; text-decoration: none;">YourWebsite</a>.<br/>
...
export { createWorkspace, getWorkspaces, getWorkspaceDetails, getWorkspaceProjects, getWorkspaceStats, inviteUserToWorkspace, acceptGenerateInvite, acceptInviteByToken }